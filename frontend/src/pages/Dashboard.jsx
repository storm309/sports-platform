...
const PerformanceChart = ({ data }) => (
  <div className="h-full w-full flex items-end justify-between gap-2 px-2 pb-2">
    {data.length === 0 ? (
      ...
      data.map((d, i) => (
        <div key={i} className="group relative w-full h-[80%] bg-slate-800/50 rounded-t-lg hover:bg-slate-700/50 transition-all">
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans relative overflow-hidden p-4 md:p-6">
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center bg-slate-900/50 backdrop-blur-xl p-4 rounded-2xl border border-white/5 shadow-lg">
          ...
          <button
            onClick={logout}
            className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-red-500/10 text-slate-300 hover:text-red-400 border border-slate-700 hover:border-red-500/50 rounded-xl transition-all duration-300 group"
          >
        ...
        <div className="grid lg:grid-cols-12 gap-6">
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    required
                    className="w-full p-3 rounded-xl bg-slate-800/50 border border-slate-700/50 focus:border-blue-500/50 focus:bg-slate-800 outline-none transition-all placeholder-slate-600"
                  />
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="space-y-1">
                     ...
                     <div className="relative">
                        <Zap className="w-3 h-3 absolute left-3 top-3.5 text-yellow-400" />
                        <input
                          name="speed"
                          value={form.speed}
                          onChange={handleChange}
                          placeholder="0-100"
                          type="number"
                          className="w-full pl-8 pr-2 py-2.5 rounded-xl bg-slate-800/50 border border-slate-700/50 focus:border-yellow-500/50 outline-none text-center text-sm transition-all"
                        />
                     </div>
                  </div>
                  <div className="space-y-1">
                     ...
                     <div className="relative">
                        <Heart className="w-3 h-3 absolute left-3 top-3.5 text-red-400" />
                        <input
                          name="stamina"
                          value={form.stamina}
                          onChange={handleChange}
                          placeholder="0-100"
                          type="number"
                          className="w-full pl-8 pr-2 py-2.5 rounded-xl bg-slate-800/50 border border-slate-700/50 focus:border-red-500/50 outline-none text-center text-sm transition-all"
                        />
                     </div>
                  </div>
                  <div className="space-y-1">
                     ...
                     <div className="relative">
                        <Dumbbell className="w-3 h-3 absolute left-3 top-3.5 text-purple-400" />
                        <input
                          name="strength"
                          value={form.strength}
                          onChange={handleChange}
                          placeholder="0-100"
                          type="number"
                          className="w-full pl-8 pr-2 py-2.5 rounded-xl bg-slate-800/50 border border-slate-700/50 focus:border-purple-500/50 outline-none text-center text-sm transition-all"
                        />
                     </div>
                  </div>
                </div>
                ...
                <div className="space-y-3 pt-2">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 p-3 rounded-xl bg-slate-800/50 border border-slate-700/50 focus:border-blue-500/50 outline-none transition-all text-sm placeholder-slate-600"
                    />
                  </div>
                  ...
                  <div className="relative group cursor-pointer">
                    ...
                    <label 
                      htmlFor="file-upload"
                      className={`flex flex-col items-center justify-center w-full p-4 border-2 border-dashed rounded-xl cursor-pointer transition-all ${videoFile ? 'border-green-500/50 bg-green-500/10' : 'border-slate-700/50 hover:border-blue-500/50 hover:bg-slate-800/50'}`}
                    >
                      {videoFile ? (
                ...
                <button
                  ...
                  className="w-full py-3.5 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-semibold rounded-xl shadow-lg shadow-blue-600/20 hover:shadow-blue-600/40 transition-all duration-300 transform active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                >
                  {isSubmitting ? "Saving..." : "Save Performance"}
                </button>

                {msg && (
                  <div className={`p-3 rounded-xl text-center text-sm flex items-center justify-center gap-2 ${msg.includes("Error") ? "bg-red-500/10 text-red-400" : "bg-green-500/10 text-green-400"}`}>
                    {msg.includes("Error") ? <AlertCircle className="w-4 h-4"/> : <CheckCircle2 className="w-4 h-4"/>}
                    {msg}
                  </div>
                )}
              </form>
          ...
          {/* RIGHT COLUMN - CHART & LIST (8 Cols) */}
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-slate-900/60 backdrop-blur-2xl p-6 rounded-3xl border border-white/10 shadow-xl h-[300px] flex flex-col">
              ...
              <div className="flex-1 bg-slate-800/30 rounded-xl border border-slate-700/30 pt-4">
              ...
              <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-3">
                {list.length === 0 ? (
                  ...
                  list.map((p) => (
                    <div
                      key={p._id}
                      className="group bg-slate-800/40 hover:bg-slate-800/80 border border-slate-700/50 hover:border-blue-500/30 p-4 rounded-2xl transition-all duration-300"
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <h3 className="font-bold text-white text-lg">{p.sport}</h3>
                          <p className="text-xs text-slate-500">
                            {new Date(p.createdAt).toLocaleDateString(undefined, { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}
                          </p>
                        </div>
                        ...
                        <div className="flex gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                          {p.videoUrl && (
                            <a href={p.videoUrl} target="_blank" rel="noreferrer" className="p-2 bg-slate-700 rounded-lg hover:bg-blue-600 hover:text-white transition-colors" title="External Link">
                              <LinkIcon className="w-4 h-4" />
                            </a>
                          )}
                          {p.videoFile && (
                            <a href={`http://localhost:5000${p.videoFile}`} target="_blank" rel="noreferrer" className="p-2 bg-slate-700 rounded-lg hover:bg-teal-600 hover:text-white transition-colors" title="View Upload">
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-2">
                        <div className="bg-slate-900/50 rounded-lg p-2 flex items-center justify-between border border-slate-700/30">
                          <Zap className="w-3 h-3 text-yellow-500" />
                          <span className="text-sm font-bold">{p.speed}</span>
                        </div>
                        <div className="bg-slate-900/50 rounded-lg p-2 flex items-center justify-between border border-slate-700/30">
                          <Heart className="w-3 h-3 text-red-500" />
                          <span className="text-sm font-bold">{p.stamina}</span>
                        </div>
                        <div className="bg-slate-900/50 rounded-lg p-2 flex items-center justify-between border border-slate-700/30">
                          <Dumbbell className="w-3 h-3 text-purple-500" />
                          <span className="text-sm font-bold">{p.strength}</span>
                        </div>
                      </div>
                    </div>
                  ))
