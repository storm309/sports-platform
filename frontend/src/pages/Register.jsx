...
import { User, Lock, Mail, ArrowRight, Hexagon, AlertCircle, CheckCircle2, Sparkles, Eye, EyeOff } from "lucide-react";
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (data.email === "test@test.com") {
          reject({ response: { data: { message: "User already exists with this email." } } });
        } else {
          resolve({
            data: {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-950 text-slate-100 font-sans relative overflow-hidden px-4 py-8">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute top-[-10%] right-[-5%] w-[500px] h-[500px] bg-blue-500/30 rounded-full blur-[120px] animate-pulse" />
        <div className="absolute bottom-[-10%] left-[-5%] w-[600px] h-[600px] bg-purple-500/20 rounded-full blur-[120px] animate-pulse" style={{ animationDelay: '1s' }} />
        ...
        <div className="relative bg-slate-900/80 backdrop-blur-2xl p-8 sm:p-10 rounded-3xl shadow-2xl border border-white/10">
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl sm:text-4xl font-black tracking-tight mb-2">
              <span className="bg-gradient-to-r from-white via-blue-100 to-blue-200 bg-clip-text text-transparent">Create Account</span>
            </h2>
            <p className="text-slate-400 text-sm">Join us to track and analyze your performance</p>
          </div>
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input name="name" placeholder="John Doe" type="text" required value={form.name} className="w-full pl-12 pr-4 py-3.5 bg-slate-800/50 border border-slate-700/50 rounded-xl text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:bg-slate-800 transition-all" onChange={handleChange} />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <label className="cursor-pointer">
                  ...
                  <div className={`p-3.5 text-center rounded-xl border transition-all ${form.role === 'player' ? 'bg-blue-600/20 border-blue-500 text-blue-400' : 'bg-slate-800/50 border-slate-700/50 text-slate-400'}`}>
                    <span className="font-medium">Player</span>
                  </div>
                </label>
                <label className="cursor-pointer">
                  <input type="radio" name="role" value="coach" checked={form.role === 'coach'} onChange={handleChange} className="hidden" />
                  <div className={`p-3.5 text-center rounded-xl border transition-all ${form.role === 'coach' ? 'bg-purple-600/20 border-purple-500 text-purple-400' : 'bg-slate-800/50 border-slate-700/50 text-slate-400'}`}>
          ...
          <div className="mt-8 text-center pt-6 border-t border-slate-700/50">
            <p className="text-slate-400 text-sm">Already have an account? <Link to="/login" className="text-blue-400 hover:text-blue-300 font-semibold hover:underline">Sign in here</Link></p>
            <p className="text-xs text-slate-600 mt-2">Need help? <a href="mailto:shivamkumarp447@gmail.com" className="text-slate-500 hover:text-slate-400 underline">Contact Support</a></p>
          </div>
        </div>
      </div>
    </div>
